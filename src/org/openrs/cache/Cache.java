...
import org.openrs.cache.util.ByteBufferUtils;
import org.openrs.cache.util.crypto.BKDR;
import org.openrs.cache.util.crypto.Whirlpool;
...
public final class Cache implements Closeable {
	...
	public Cache(FileStore store) {
		...
		Container container;
		try {
			container = new Container(Container.COMPRESSION_NONE, createChecksumTable().encode());
			table = container.encode();
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * Finds a name of a certain file in the reference table.
	 * @param table The reference table.
	 * @param name The name.
	 * @return The name as a hash.
	 */
