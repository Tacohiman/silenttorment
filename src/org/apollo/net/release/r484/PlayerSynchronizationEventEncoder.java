...
public final class PlayerSynchronizationEventEncoder extends EventEncoder<PlayerSynchronizationEvent> {
	...
	private void putBlocks(SynchronizationSegment segment, GamePacketBuilder blockBuilder) {
		...
		if (blockSet.size() > 0) {
			...
			if (blockSet.contains(HitBlock.class))
				putHitBlock(blockSet.get(HitBlock.class), blockBuilder);
			...
			if (blockSet.contains(SecondHitBlock.class))
				putSecondHitBlock(blockSet.get(SecondHitBlock.class), blockBuilder);
		}
	}
	
	private void putSecondHitBlock(SecondHitBlock block, GamePacketBuilder blockBuilder) {
		Hit hit = block.getHit();
		
		blockBuilder.put(DataType.BYTE, hit.getDamageReceived());
		blockBuilder.put(DataType.BYTE, hit.getHitType().toInteger());
	}
	
	private void putHitBlock(HitBlock block, GamePacketBuilder blockBuilder) {
		Hit hit = block.getHit();
		
		blockBuilder.put(DataType.BYTE, DataTransformation.NEGATE, hit.getDamageReceived());
		blockBuilder.put(DataType.BYTE, DataTransformation.NEGATE, hit.getHitType().toInteger());
		blockBuilder.put(DataType.BYTE, DataTransformation.ADD, (block.getCurrentHitpoints() * 30) / block.getMaximumHitpoints());
	}
