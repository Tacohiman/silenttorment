...
public final class Player extends Character {
	...
	private void sendInitialEvents() {
		send(new RegionChangeEvent(getPosition()));
		send(new RootInterfaceEvent(InterfaceSet.DEFAULT_ROOT_INTERFACE));
		int[][] tabs = new int[][] {
				new int[] { 121, 128, 129, 130, 131, 132, 133, 134, 135, 136, 137,
						138, 139, 140, 141, 142 },
						new int[] { 137, 92, 320, 274, 149, 387, 271, 192, 662, 550, 551,
						589, 261, 464, 239, 182 }, };
		for (int i = 0; i < tabs[0].length; i++) 
			send(new OpenInterfaceEvent(tabs[1][i], tabs[0][i]));
		getSkillSet().forceRefresh();
		sendPlayerOptions("Attack", "Follow", "Trade");
	}
	
	/**
	 * Done because most options update unanimously.
	 * 
	 * @param options
	 *            The options you would like to send respectively (in order).
	 */
	public void sendPlayerOptions(String...options) {
		for (int slot = 1; slot < options.length; slot++) 
			send(new PlayerOptionEvent(options[slot], slot, options.equals(options[0])));
	}
